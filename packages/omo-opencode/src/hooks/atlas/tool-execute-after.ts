...
export function createToolExecuteAfterHandler(input: {
  ...
  const { ctx, pendingFilePaths, pendingTaskRefs, pendingPlanSnapshots, autoCommit, disableFinalVerificationWave, getState } = input
  ...
  return async (toolInput, toolOutput): Promise<void> => {
    ...
    // Plan stripping runs for ALL sessions (including Prometheus), not just Atlas
    if (disableFinalVerificationWave && isWriteOrEditToolName(toolInput.tool)) {
      const filePath = toolInput.callID
        ? pendingFilePaths.get(toolInput.callID) ?? (toolOutput.metadata?.filePath as string | undefined)
        : (toolOutput.metadata?.filePath as string | undefined)
      if (filePath && isPlanFilePath(ctx.directory, filePath)) {
        stripFinalVerificationWaveFromPlan(filePath)
      }
    }
    ...
    if (toolOutput.output && typeof toolOutput.output === "string") {
      ...
      if (boulderState) {
        ...
        const shouldPauseForApproval = !disableFinalVerificationWave && sessionState
